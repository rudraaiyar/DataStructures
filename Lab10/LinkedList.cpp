...
LinkedList::LinkedList()			// this is the first default constructor that just sets the first and last to NULL value
{
    first = NULL;
    last = NULL;
}

LinkedList::~LinkedList()			// this is the  destructor class that just emptys the list after we are done using it
{
    clear ();
}

void LinkedList::insertAtBack (int initialValue)// this is the function that will insert a new node at the back of the linked list
{
    Node*temp = new Node();
...
bool LinkedList::removeFromBack()		//this is the function that will remove a node from the back of the linked list
{
    Node*temp = first;
    if (last == NULL ) {
        return false;
...
void LinkedList::print ()			//this is the function that will print everything from the LinkedList.h file
{
    if (first != NULL ) {
        Node*temp = first;
        while (temp != last){
...
bool LinkedList::isEmpty ()			//this function is only checking to see if the linked list is empty or not
{
...
int LinkedList::size ()				//this function is checking the size of the linked list
{
    Node *temp;
...
void LinkedList :: clear()			//this function will clear the entire linked list if it not already empty
{
    while (!isEmpty()){
...
void LinkedList :: insertAtFront (int initialValue)// this function is going to insert a new node at the beginning of the linked list
{
    Node*temp = new Node();
    temp -> val = initialValue;
    if (first == NULL){
    ...
    else {					//for this function it is practically the same as the remove from back, except we do not have to check if it is the front becuase it is already pointing at the front
        temp -> next = first;
        first = temp;
...
bool LinkedList :: removeFromFront(){		//this function is going to remove a node fro mthe beginning of the linked list
    Node*temp = first;
